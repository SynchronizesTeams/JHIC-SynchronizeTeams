...
export const useGuestBook = () => {
  ...
  const fetchGuestBooks = async () => {
    ...
    try {
      const response: any = await api.guestBook.getAll();
  ...
  const fetchGuestBookById = async (id: number) => {
    ...
    try {
      const response: any = await api.guestBook.getById(id);
  ...
  const createGuestBook = async (guestBookData: {
    ...
    try {
      const response: any = await api.guestBook.create(guestBookData);
  ...
  const updateGuestBookStatus = async (
    id: number,
    ...
    try {
      const response: any = await api.guestBook.updateStatus(id, status);

      // Update local state
      const index: any = guestBooks.value.findIndex((gb) => gb.id === id);
      if (index !== -1) {
  ...
  const getPendingGuestBooks = () => {
    return guestBooks.value.filter(
      (gb) => gb.status === "pending" || !gb.status
    );
  };
