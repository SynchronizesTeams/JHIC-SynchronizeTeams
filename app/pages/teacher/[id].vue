<template>
  <div class="min-h-screen bg-gradient-to-br from-primary-white to-white">
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div class="bg-primary-white border border-primary-gray/20 rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow mb-6">
        <!-- Cover Background -->
        <div class="h-32 bg-gradient-to-r from-secondary-red/20 to-secondary-red/10">
          <NuxtImg
        ...
        <div class="px-6 py-8 text-center">
          ...
          <!-- Name & Position -->
          <h1 class="text-2xl font-bold text-primary-gray mb-1">{{ teacher?.full_name }}</h1>
          <p class="text-secondary-red text-sm mb-2">{{ teacher?.position }} {{ teacher?.subject }}</p>

          <!-- NIG & Username -->
          <div class="flex flex-col sm:flex-row gap-2 justify-center items-center text-xs text-primary-gray/60 mb-4">
      ...
      <!-- Description Card -->
      <div class="bg-primary-white border border-primary-gray/20 rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow">
        <!-- Header -->
      ...
      <div class="mt-6 flex justify-center">
        <div class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-secondary-red/10 to-secondary-red/5 border border-secondary-red/20 rounded-full">
          ...
          <span class="text-sm font-semibold text-secondary-red">Mengajar: {{ teacher?.subject }}</span>
        </div>
      </div>
    </div>
  </div>
...
// Mock: Find teacher by ID
const teacher = computed(() => {
  return mockTeachers.find(t => t.id === teacherId.value)
})
...
useHead({
  ...
  meta: [
    { name: 'description', content: teacher.value?.description || `Profil ${teacher.value?.full_name}, Guru ${teacher.value?.subject}` }
  ]
})
</script>
